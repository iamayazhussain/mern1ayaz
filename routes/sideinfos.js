...
const Sideinfo = require('../models/Sideinfo')
...
router.get('/', async (req, res) => {
  try {
    const infos = await Sideinfo.find().sort({ date: -1 })
...
router.put('/:id', auth, async (req, res) => {
  ...
  try {
    let info = await Sideinfo.findById(req.params.id)

    if (!info) return res.status(404).json({ msg: 'Info not found' })

    info = await Sideinfo.findByIdAndUpdate(
      req.params.id,
      { $set: infoFields },
      { new: true },
    )
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    await Sideinfo.findByIdAndRemove(req.params.id)
